...
@Slf4j
@Component
public class JWTProspectAuthGatewayFilter extends AbstractGatewayFilterFactory<JWTProspectAuthGatewayFilter.Config> {

    private final JWKSourceCacheProvider jwkSourceCacheProvider;

    public JWTProspectAuthGatewayFilter(JWKSourceCacheProvider jwkSourceCacheProvider) {
        super(Config.class);
    ...
    public GatewayFilter apply(Config config) {
        return (exchange, chain) -> {
            ...
            try {
                ...
                String userRolesStr = String.join("#", claims.getStringListClaim("roles"));
                ...
                String accessLevel = resolvedApiRole.stream().findFirst().get().split("\\|")[1];

                ServerHttpRequest mutatedRequest = exchange.getRequest()
                        .mutate()
                        .header("X-Access-Level", accessLevel)
                        .header("X-User-Tenant", tenant)
                        .header("X-User-Role", userRolesStr)
