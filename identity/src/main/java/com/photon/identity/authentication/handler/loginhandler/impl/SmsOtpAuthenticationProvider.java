...
@Slf4j
@Component("smsOtpAuthenticationProvider")
public class SmsOtpAuthenticationProvider implements PhotonAuthenticationProvider {

    private final AuthenticationManager authenticationManager;
    private final TempCredentialRepository tempCredentialRepository;
    private final UserRepository userRepository;

    public SmsOtpAuthenticationProvider(AuthenticationManager authenticationManager, TempCredentialRepository tempCredentialRepository, UserRepository userRepository) {
    ...
    public Authentication authenticate(IdentityProviderDto identityProvider, AuthTypeDto authType, Object authRequest) throws ApplicationException {
        ...
        SmsAuthConfigDto authConfig = (SmsAuthConfigDto) authType.getConfig();
        ...
        Authentication token = switch (identityProvider.getIdentityProviderType()) {
            case LOGIN -> new SmsOtpAuthenticationToken(userDetails.get(), tempCredential.get(), verifyOtp.getSecurityKey(), verifyOtp.getPassword());
            case ON_BOARDING -> new SmsOtpAuthenticationToken(tempCredential.get(), verifyOtp.getSecurityKey(), verifyOtp.getPassword());
